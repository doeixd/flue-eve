...
authoring can keep the familiar Eve-style `agent/` file layout; the scaffold
imports those declarations into Flue agents and tools.
...
- Eve filesystem discovery: `agent/instructions.md`, `agent/tools/*`, and
  `agent/connections/*` are supported through scaffold/import. Flue still owns
  runtime discovery, so review generated files when you change the layout.
