...
export async function openStreamBody(input: OpenStreamBodyInput): Promise<ReadableStream<Uint8Array>> {
  ...
  for (let attempt = 0; attempt < STREAM_OPEN_RETRY_ATTEMPTS; attempt += 1) {
    ...
    const response = await fetch(url, { headers, signal: input.signal ?? null });
