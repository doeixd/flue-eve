// Adapted from vercel/eve packages/eve/src/client/message-reducer.test.ts (Apache-2.0)
...
import { defaultMessageReducer } from "./message-reducer.js";
...
describe("defaultMessageReducer", () => {
  it("projects messages, reasoning, and actions into UIMessage-compatible parts", () => {
    ...
    data = reducer.reduce(data, {
      data: { createdAt: 1, message: "Weather in Vienna?", submissionId: "submission_1" },
    ...
    expect(data.messages).toEqual([
      ...
      {
        id: "turn_1:assistant",
        metadata: { status: "streaming", turnId: "turn_1" },
        parts: [
          { type: "step-start" },
          {
            state: "done",
          ...
          },
          {
            ...
            stepIndex: 0,
            toolCallId: "call_1",
            toolMetadata: { eve: { kind: "tool-call", name: "get_weather" } },
            toolName: "get_weather",
            type: "dynamic-tool",
          },
        ],
    ...
    expect(data.messages).toEqual([
      {
        ...
        parts: [
          { type: "step-start" },
          {
            ...
            stepIndex: 0,
            toolCallId: "call_1",
            toolMetadata: { eve: { kind: "subagent-call", name: "research" } },
    ...
    expect(data.messages).toEqual([
      {
        ...
        parts: [
          { type: "step-start" },
          {
            approval: { approved: false, id: "call_1", reason: "Tool execution was denied." },
            input: undefined,
            state: "output-denied",
            stepIndex: 0,
            toolCallId: "call_1",
  ...
  it("stores completed structured results on assistant metadata", () => {
    ...
    expect(data.messages).toEqual([
      {
        id: "turn_1:assistant",
        metadata: { result: { title: "Done" }, status: "streaming", turnId: "turn_1" },
        parts: [],
        role: "assistant",
      },
    ]);
  ...
  it("marks input requests as responded when the client submits a response", () => {
    ...
    data = reducer.reduce(data, {
      data: { createdAt: 1, responses: [{ optionId: "deny", requestId: "approval_1" }] },
  ...
  it("merges resumed approval results back into the requested tool part", () => {
    ...
    data = reducer.reduce(
      data,
      createStepStartedEvent({ sequence: 1, stepIndex: 0, turnId: "turn_1" }),
    );
    data = reducer.reduce(
      data,
      createActionResultEvent({
    ...
    expect(toolParts[0]).toMatchObject({
      ...
      toolMetadata: {
        eve: {
          inputRequest: { prompt: "Approve tool call: bash", requestId: "approval_1" },
    ...
    expect(data.messages).toEqual([
      {
        ...
        parts: [
          { type: "step-start" },
          { state: "done", stepIndex: 0, text: "First step.", type: "text" },
          { type: "step-start" },
          { state: "done", stepIndex: 1, text: "Second step.", type: "text" },
        ],
        role: "assistant",
      },
    ]);
  ...
  it("completes the correct text part when message.completed(null) arrives with stepIndex", () => {
    const reducer = defaultMessageReducer();
    let data = reducer.initial();
    ...
    expect(textParts[1]!.state).toBe("streaming");
  });

  it("falls back to last text part when no matching step exists for completeExistingTextPart", () => {
    const reducer = defaultMessageReducer();
    let data = reducer.initial();
